  ...
  return (
    <div className='bg-gray-200 p-5'>
        ...
        <div className='flex justify-between mb-5'>
            <h1 className=' text-black text-3xl font-bold'>POPULAR TOUR PACKAGES</h1>
            <button className='text-blue text-xl font-semibold'>VIEW ALL</button>
        </div>
        <div>
            {
                travelData.map((e:travelInfoType, index:number)=>{
                    ...
                    <div className={`w-full h- mb-5 bg-white rounded-lg flex justify-between ${index<=counter?"null":"hidden"}`}>
                        <div className='p-3 flex gap-3 relative w-10/12'>
                            <img src={`${e.image.mainImage}`} alt="asdas"  className=' rounded-lg w-3/12 h-40'/>
                            <div className='w-9/12'>
                                <h1 className='text-2xl text-black' >{e.name}</h1>
                                <p>{e.additionalInfo}</p>
