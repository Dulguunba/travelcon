...
export const GeneralAdmin = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      destination: Yup.string().max(50, "Must be 50 characters or less")
      .required("Travel company name is required")
    }),
    onSubmit: (values) => {
      console.log("test``");
        ...
        <div className="flex gap-5 w-full">
          ...
          <div className="w-1/2 flex flex-col gap-2">
            <div className="w-full h-20 text-sm flex-col  gap-1">
              ...
              <select id="destination" {...formik.getFieldProps("destination")} className="bg-gray-100 rounded-lg w-1/2 p-1 px-3 text-sm h-10 border border-gray-200 capitalize">
                {destinations?.map((destination: {_id: string, name: string})=> 
                  <option value={destination._id} className="capitalize">{destination.name}</option>
                )}
              </select>
